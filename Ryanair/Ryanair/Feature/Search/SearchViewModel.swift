...
extension SearchViewModel: SearchViewModelProtocol {
    ...
    func bindFlights() -> AnyPublisher<[Flight], RyanairError> {
        let tripsSubscriber = PassthroughSubject<[Flight], RyanairError>()
        service.getFlights()
            .sink(receiveCompletion: { completion in
                switch completion {
                case .failure(let error):
                    tripsSubscriber.send(completion: .failure(error))
                case .finished: print("")
                }
            }, receiveValue: { flights in
                tripsSubscriber.send(flights)
            }).store(in: &cancellable)
        return tripsSubscriber.eraseToAnyPublisher()
    }
}
