...
extension RyanairService: RyanairServiceProtocol {
    ...
    func getFlight() -> AnyPublisher<Flight, RyanairError> {
        let flightSubject = PassthroughSubject<Flight, RyanairError>()
        URLSession.shared.dataTaskPublisher(for: getRequest(with: .flight))
            .tryMap { data, response -> Data in
                self.validateFields(data: data, response: response, subject: flightSubject)
                return data
            }
